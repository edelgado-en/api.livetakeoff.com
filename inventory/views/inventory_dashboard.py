...
class InventoryDashboardView(APIView):
    ...
    def post(self, request):
        ...
        if start_date and end_date:
            total_inventory_value = LocationItemActivity.objects.filter(activity_type='A') \
                                              .filter(timestamp__range=(start_date, end_date)) \
                                              .aggregate(Sum('cost'))['cost__sum']
            
            total_cost_subtracted = LocationItemActivity.objects.filter(activity_type='S') \
                                                .filter(timestamp__range=(start_date, end_date)) \
                                                .aggregate(Sum('cost'))['cost__sum']
            
            total_expense = total_cost_subtracted

            total_items_added = LocationItemActivity.objects.filter(activity_type='A') \
                                                .filter(timestamp__range=(start_date, end_date)) \
                                                .aggregate(Sum('quantity'))['quantity__sum']

            total_items_subtracted = LocationItemActivity.objects.filter(activity_type='S') \
                                                .filter(timestamp__range=(start_date, end_date)) \
                                                .aggregate(Sum('quantity'))['quantity__sum']

            total_items_in_stock = total_items_added - total_items_subtracted

            total_location_item_activities = LocationItemActivity.objects \
                                                    .filter(timestamp__range=(start_date, end_date)).count()

            # Get the top 10 most popular items. 
            # A popular item is defined by the number of locationItemActivities regardless of the activity_type
            # the results set should include the item name and the corresponding number of locationItemActivities sorted by highest number of locationItemActivities
            qs = LocationItemActivity.objects.filter(timestamp__range=(start_date, end_date)) \
                                                .values('location_item__item__name') \
                                                .annotate(count=Count('location_item__item__name')) \
                                                .order_by('-count')[:10]
            for item in qs:
            ...
            qs = LocationItemActivity.objects.filter(timestamp__range=(start_date, end_date)) \
                                             .values('user__id') \
                                             .annotate(total_transactions=Count('user__id'),
            ...
            total_inventory_value = qs.aggregate(Sum('cost'))['cost__sum']
            ...
            total_location_item_activities = LocationItemActivity.objects.all().count()

            # get the top 10 most popular items
            qs = LocationItemActivity.objects.values('location_item__item__name') \
                                                .annotate(count=Count('location_item__item__name')) \
                                                .order_by('-count')[:10]
